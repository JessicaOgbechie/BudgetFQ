...
function FAQModal({ onClose }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      ...
      {/* Modal */}
      <div style={{ position: 'relative', background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 16, width: '100%', maxWidth: 560, maxHeight: '80vh', overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 8px 40px rgba(0,0,0,0.12)' }}>
        {/* Header */}
        ...
        <div style={{ overflowY: 'auto', padding: '12px 20px 20px' }}>
          {FAQ_ITEMS.map((item, i) => (
            <div key={i} style={{ borderBottom: i < FAQ_ITEMS.length - 1 ? '1px solid var(--border)' : 'none', marginBottom: 0 }}>
              <button
                onClick={() => setOpen(open === i ? null : i)}
                style={{ width: '100%', background: 'none', border: 'none', padding: '14px 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, cursor: 'pointer', textAlign: 'left' }}
              >
              ...
              {open === i && (
                <div style={{ fontSize: 13, color: 'var(--text-body)', lineHeight: 1.7, paddingBottom: 14 }}>
                  {item.a}
                </div>
              )}
            </div>
...
export default function Header({ primaryCurrency, setPrimaryCurrency, state, totalIncome }) {
  ...
  const selectedCurrency = CURRENCIES.find(c => c.code === primaryCurrency) || CURRENCIES[0];
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, position: 'relative' }}>
          ...
          <div style={{ position: 'relative' }}>
            <select
              ...
              style={{ background: 'var(--bg-input)', border: '1px solid var(--border)', borderRadius: 8, color: 'var(--text-primary)', fontFamily: 'DM Sans, sans-serif', fontSize: 13, padding: '7px 10px', cursor: 'pointer', paddingLeft: 30, appearance: 'none', WebkitAppearance: 'none' }}
            >
              {CURRENCIES.map(c => (
          ...
          {menuOpen && (
            <>
              ...
              <div style={{ position: 'absolute', top: 42, right: 0, zIndex: 201, background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 12, padding: 6, minWidth: 170, boxShadow: '0 4px 20px rgba(0,0,0,0.10)' }}>
                {[
                  { label: 'Export CSV', icon: 'ti-download', action: handleCSV },
                  { label: 'Backup data', icon: 'ti-database-export', action: handleJSONExport },
                ].map(item => (
                  <button key={item.label} onClick={item.action}
                    style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '10px 12px', borderRadius: 8, border: 'none', background: 'transparent', fontFamily: 'DM Sans, sans-serif', fontSize: 14, color: 'var(--text-body)', cursor: 'pointer', textAlign: 'left' }}
                    onMouseEnter={e => e.currentTarget.style.background = 'var(--bg-hover)'}
                    onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
                  >
                    <i className={`ti ${item.icon}`} aria-hidden="true" style={{ fontSize: 16, color: 'var(--accent)' }} />
                    {item.label}
                  </button>
                ))}
              </div>
